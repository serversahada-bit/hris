...
import { updateProfilKaryawan, updateAkunKaryawan, tambahRiwayatKarir, berhentikanKaryawan } from "@/app/actions/karyawan";
...
export default function DetailClient({ karyawan, riwayatKarir, presensi, izinCuti }: {
  ...
  const [isSubmittingKarir, setIsSubmittingKarir] = useState(false);
  const [isBerhentikanModalOpen, setIsBerhentikanModalOpen] = useState(false);
  const [isSubmittingBerhentikan, setIsSubmittingBerhentikan] = useState(false);
  ...
  // Handle Update Karir Submit
  const handleKarirSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setIsSubmittingKarir(true);
    const formData = new FormData(e.currentTarget);
    const res = await tambahRiwayatKarir(formData);
    setIsSubmittingKarir(false);
  ...
  // Handle Berhentikan Submit
  const handleBerhentikanSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setIsSubmittingBerhentikan(true);
    const formData = new FormData(e.currentTarget);
    const res = await berhentikanKaryawan(formData);
    setIsSubmittingBerhentikan(false);

    notify(res.message, res.success ? "success" : "error");
    if (res.success) {
      setIsBerhentikanModalOpen(false);
      router.refresh();
    }
  };
  ...
  return (
    <>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-8 gap-x-6">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-slate-800 dark:text-slate-200">{karyawan.pendidikan_terakhir || '-'}</p>
                    </div>
                  </div>
                </div>
              ...
              <div className="animate-in fade-in duration-300">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 border-b border-slate-100 dark:border-slate-700 pb-4">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => setIsKarirModalOpen(true)}
                      className="h-10 px-4 rounded-xl bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition text-sm"
                    >
                      Update Karir
                    </button>
                    <button
                      onClick={() => setIsBerhentikanModalOpen(true)}
                      disabled={!statusAktif}
                      className="h-10 px-4 rounded-xl bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 font-bold border border-rose-200 dark:border-rose-800/30 hover:bg-rose-100 dark:hover:bg-rose-500/20 transition text-sm disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-rose-50 dark:disabled:hover:bg-rose-500/10"
                    >
                      {statusAktif ? "Berhentikan" : "Sudah Berhenti"}
                    </button>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      ...
                      <div>
                        ...
                        <select name="agama" defaultValue={karyawan.agama || ""} className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm">
                      ...
                      <div>
                        ...
                        <select name="golongan_darah" defaultValue={karyawan.golongan_darah || ""} className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm">
                      ...
                      <div>
                        ...
                        <select name="warga" defaultValue={karyawan.warga || "WNI"} className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm">
                      ...
                      <div>
                        ...
                        <select name="pendidikan_terakhir" defaultValue={karyawan.pendidikan_terakhir || ""} className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      ...
                      <div>
                        ...
                        <input type="text" name="kontak_darurat" defaultValue={karyawan.kontak_darurat} className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm" />
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <input type="text" name="nomor_kk" defaultValue={karyawan.nomor_kk} className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm" />
                      </div>
                      <div>
                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">NPWP</label>
                        <input type="text" name="npwp" defaultValue={karyawan.npwp} className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm" />
                      </div>
                      <div className="md:col-span-2 lg:col-span-3">
                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Alamat KTP</label>
                        <textarea name="alamat_kartu_identitas" rows={2} defaultValue={karyawan.alamat_kartu_identitas} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm resize-none"></textarea>
                      </div>
                      <div className="md:col-span-2 lg:col-span-3">
                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Alamat Domisili</label>
                        <textarea name="alamat_domisili" rows={2} defaultValue={karyawan.alamat_domisili} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm resize-none"></textarea>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      <div>
                        ...
                        <input type="text" name="bank" defaultValue={karyawan.bank} className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm" />
                      </div>
                      <div>
                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">BPJS Kesehatan</label>
                        <input type="text" name="bpjs_kesehatan" defaultValue={karyawan.bpjs_kesehatan} className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm" />
                      </div>
                      <div>
                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">BPJS Ketenagakerjaan</label>
                        <input type="text" name="bpjs_ketenagakerjaan" defaultValue={karyawan.bpjs_ketenagakerjaan} className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm" />
                      </div>
                    </div>
                  </div>
      ...
      {/* Update Karir Modal */}
      {mounted && isKarirModalOpen && createPortal(
        <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 sm:p-6" style={{ isolation: 'isolate' }}>
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity" onClick={() => !isSubmittingKarir && setIsKarirModalOpen(false)}></div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col relative z-10 animate-in zoom-in-95 duration-200 border border-slate-200/50 dark:border-slate-700 overflow-hidden">
            ...
            <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/80 dark:bg-slate-800/80 shrink-0">
              <div>
                <h3 className="font-extrabold text-slate-900 dark:text-white text-xl">Update Karir</h3>
                <p className="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Catat promosi, mutasi, atau perubahan status karyawan.</p>
              </div>
              <button
                onClick={() => setIsKarirModalOpen(false)}
                disabled={isSubmittingKarir}
                className="h-10 w-10 rounded-full flex items-center justify-center text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-600 transition focus:outline-none"
              >
                <span className="material-symbols-outlined text-[24px]">close</span>
              </button>
            ...
            <form onSubmit={handleKarirSubmit} className="flex flex-col flex-1 overflow-hidden">
              <input type="hidden" name="karyawan_id" value={karyawan.id} />

              <div className="p-8 overflow-y-auto flex-1 space-y-5 custom-scrollbar">
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Tipe Perubahan</label>
                  <select name="jenis_perubahan" required defaultValue="Promosi" className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm">
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Tanggal Efektif</label>
                  <input type="date" name="tanggal_efektif" required className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm" />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Pangkat</label>
                    <input type="text" name="pangkat" placeholder="Opsional" className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Status Kepegawaian</label>
                    <select name="status_karyawan" defaultValue={karyawan.status_karyawan} className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm">
              ...
              <div className="px-8 py-5 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-4 bg-slate-50/80 dark:bg-slate-800/80 shrink-0">
                <button type="button" onClick={() => setIsKarirModalOpen(false)} disabled={isSubmittingKarir} className="h-11 px-6 rounded-xl text-sm font-extrabold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600 transition shadow-sm hover:shadow focus:outline-none">
                  Batal
                </button>
                <button type="submit" disabled={isSubmittingKarir} className="h-11 px-8 rounded-xl text-sm font-extrabold text-white bg-gradient-to-r from-violet-600 to-violet-500 hover:from-violet-700 hover:to-violet-600 disabled:opacity-70 flex items-center gap-2 transition shadow-md hover:shadow-lg hover:-translate-y-0.5 focus:outline-none">
                  {isSubmittingKarir ? (
                    <>
                      <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                      Menyimpan...
      ...
      {/* Berhentikan Modal */}
      {mounted && isBerhentikanModalOpen && createPortal(
        <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 sm:p-6" style={{ isolation: 'isolate' }}>
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity" onClick={() => !isSubmittingBerhentikan && setIsBerhentikanModalOpen(false)}></div>

          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col relative z-10 animate-in zoom-in-95 duration-200 border border-slate-200/50 dark:border-slate-700 overflow-hidden">

            <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/80 dark:bg-slate-800/80 shrink-0">
              <div>
                <h3 className="font-extrabold text-slate-900 dark:text-white text-xl">Berhentikan Karyawan</h3>
                <p className="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Karyawan akan ditandai Non-Aktif.</p>
              </div>
              <button
                onClick={() => setIsBerhentikanModalOpen(false)}
                disabled={isSubmittingBerhentikan}
                className="h-10 w-10 rounded-full flex items-center justify-center text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-600 transition focus:outline-none"
              >
                <span className="material-symbols-outlined text-[24px]">close</span>
              </button>
            ...
            <form onSubmit={handleBerhentikanSubmit} className="flex flex-col flex-1 overflow-hidden">
              <input type="hidden" name="karyawan_id" value={karyawan.id} />

              <div className="p-8 overflow-y-auto flex-1 space-y-5 custom-scrollbar">
                <div className="rounded-xl bg-rose-50 dark:bg-rose-500/10 border border-rose-100 dark:border-rose-500/20 p-4 text-sm text-rose-900 dark:text-rose-300 font-semibold">
                  Tindakan ini akan mengubah status kepegawaian <b>{karyawan.nama}</b> menjadi Non-Aktif dan tercatat pada riwayat karir.
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Tanggal Efektif Berhenti</label>
                  <input type="date" name="tanggal_efektif" required defaultValue={new Date().toISOString().slice(0, 10)} className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Alasan / Keterangan</label>
                  <textarea name="keterangan" rows={3} placeholder="Opsional" className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:text-white transition shadow-sm resize-none"></textarea>
                </div>
              </div>

              <div className="px-8 py-5 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-4 bg-slate-50/80 dark:bg-slate-800/80 shrink-0">
                <button type="button" onClick={() => setIsBerhentikanModalOpen(false)} disabled={isSubmittingBerhentikan} className="h-11 px-6 rounded-xl text-sm font-extrabold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600 transition shadow-sm hover:shadow focus:outline-none">
                  Batal
                </button>
                <button type="submit" disabled={isSubmittingBerhentikan} className="h-11 px-8 rounded-xl text-sm font-extrabold text-white bg-gradient-to-r from-rose-600 to-rose-500 hover:from-rose-700 hover:to-rose-600 disabled:opacity-70 flex items-center gap-2 transition shadow-md hover:shadow-lg hover:-translate-y-0.5 focus:outline-none">
                  {isSubmittingBerhentikan ? (
                    <>
                      <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                      Memproses...
                    </>
