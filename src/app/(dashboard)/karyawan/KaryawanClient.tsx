...
import { tambahKaryawan } from "@/app/actions/karyawan";
...
export default function KaryawanClient({ employees }: { employees: any[] }) {
  ...
  const exportToXLS = () => {
    if (filteredEmployees.length === 0) {
      alert("Tidak ada data karyawan yang tampil untuk diekspor!");
      return;
    }
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-white dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700 p-1 shadow-sm">
            <button className="h-9 px-4 rounded text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition">
              Import
            </button>
