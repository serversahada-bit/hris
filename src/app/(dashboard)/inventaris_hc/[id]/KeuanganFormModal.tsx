...
import { saveAsetKeuangan } from "@/app/actions/asetKeuangan";
import type { KeuanganItem } from "./page";
...
export default function KeuanganFormModal({
  ...
  const [form, setForm] = useState({
    ...
    nominal: initial ? String(initial.nominal) : "",
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await saveAsetKeuangan(formData);
    ...
    <div className="fixed inset-0 z-[95] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-3xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-2xl overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
          <h3 className="text-lg font-extrabold text-slate-900 dark:text-white">{form.id ? "Edit Transaksi" : "Tambah Transaksi"}</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input type="number" min={0} step="0.01" name="nominal" required value={form.nominal} onChange={set("nominal")} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Keterangan</label>
            <input name="keterangan" required value={form.keterangan} onChange={set("keterangan")} placeholder="Contoh: Servis Rutin" className={inputClass} />
          </div>
